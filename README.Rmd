...
Customize code highlighting for [pkgdown](https://r-lib.github.io/pkgdown/index.html) site.

For me, the most typical usage is to run this code after `pkgdown::build_site()`:

```{r typical-usage, eval = FALSE}
...
- Run `highdown::tweak_ref_pages()` (with working directory being package root) to enable code highlighting on Reference pages. __Note__ that as for 2017-10-27 this still can cause incorrect highlighting if some actual code is placed just after comment.
